...
export default function AboutSection({ about, productCount = 0 }) {
  return (
    ...
    >
      ...
      <p className="mx-auto w-[min(780px,100%)] text-[1.04rem] leading-[1.75] text-ink-700">
        {about.description}
      </p>
            ...
            return (
              <div
                ...
                className={`rounded-full border border-[rgba(181,140,55,0.55)] bg-white/65 px-[10px] py-4 ${
                  highlight.label === "Produk Unggulan"
                    ? "grid min-h-[124px] place-content-center"
                    : ""
                }`}
              >
                <strong
                ...
                <span
                  className={`text-[0.92rem] text-ink-700 ${highlight.label === "Produk Unggulan" ? "mt-6 inline-block font-bold" : ""}`}
                >
                  {highlight.label}
                </span>
              </div>
